    ...
    <section id="tracking" className="py-20 bg-gray-50">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <span className="text-cyan-500 font-semibold text-lg mb-4 block">Track Your Shipment</span>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1">
                <input
                  ...
                  className="w-full px-4 py-3 border border-gray-300 rounded-full focus:ring-2 focus:ring-cyan-500 focus:border-transparent outline-none text-sm"
                />
              </div>
              <button
                onClick={handleTrack}
                disabled={isLoading || !trackingCode}
                className="bg-cyan-500 hover:bg-cyan-600 disabled:bg-gray-400 text-white px-8 py-3 rounded-full font-semibold transition-colors whitespace-nowrap cursor-pointer"
              >
                {isLoading ? (
                ...
                <div>
                  <div className="flex items-center justify-between mb-6 pb-6 border-b border-gray-200">
                    <div>
                      ...
                      <p className="text-gray-600">Current Status: <span className={`font-semibold ${trackingResult.status === 'In Transit' ? 'text-yellow-500' : trackingResult.status === 'Delivered' ? 'text-green-500' : trackingResult.status === 'On Hold' ? 'text-red-500' : 'text-cyan-500'}`}>{trackingResult.status}</span></p>
                    </div>
                  ...
                  <div className="mb-8">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm font-semibold text-cyan-500">{progress}%</span>
                    </div>
                  ...
                  <div className="grid grid-cols-1 gap-8 mb-8">
                    ...
                    <div className="bg-white border border-dashed border-gray-300 rounded-lg p-6 shadow-sm max-w-md mx-auto relative overflow-hidden w-full">
                      <div className="absolute left-0 top-0 h-full w-2 bg-cyan-500/10 rounded-l-lg"></div>
